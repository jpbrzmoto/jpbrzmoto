...
const ThemeList = () => {
	...
	const themes = themeFiles.map(theme => ({
		name: theme.replace(".json", ""), filename: theme
	}));
	...
	return (
		<>
			<ListBox filter value={selectedTheme} onChange={(e) => handleSelect(e.value)} options={themes} optionLabel="name" className="w-full" />
		</>
	)
}
